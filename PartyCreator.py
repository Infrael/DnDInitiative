...
class Character(ttk.Toplevel):
    def __init__(self, tavern, chair_number):
        ...
        self.hero_name_label = ttk.Label(self, text="Hero Name:")
        self.hero_name_label.grid(row=0, column=0)

        self.hero_name_entry = ttk.Entry(self, width=15)
        self.hero_name_entry.grid(row=0, column=1)
        self.hero_name_entry.focus_set()

        self.hero_image = ImageTk.PhotoImage(Image.open(f"Characters/default_male.jpg"))
        self.hero_label = ttk.Label(self, image=self.hero_image)
        self.hero_label.grid(row=1, column=0, columnspan=2)

        self.radio_button_value = ttk.StringVar()

        self.gender_male_radio = ttk.Radiobutton(self, text="Male", variable=self.radio_button_value,
                                                 value="male", command=self.change_hero_image)
        self.gender_male_radio.invoke()
        self.gender_male_radio.grid(row=2, column=0)

        self.gender_female_radio = ttk.Radiobutton(self, text="Female", variable=self.radio_button_value,
                                                   value="female", command=self.change_hero_image)
        self.gender_female_radio.deselect()
        self.gender_female_radio.grid(row=2, column=1)

        self.hero_image_browse_button = ttk.Button(self, text="OR, Browse For Image (jpg, png)",
                                                   command=self.browse_for_file)
        self.hero_image_browse_button.grid(row=3, column=0, columnspan=2, sticky="WE")

        self.grid_placeholder = ttk.Label(self, text="=^.^=")
        self.grid_placeholder.grid(row=4, column=0, columnspan=2)

        self.hero_image_location = "Characters/default_male.jpg"

        self.create_hero_button = ttk.Button(self, text="Add to Party", command=lambda: self.add_hero(chair_number))
    ...
    def change_hero_image(self):
        if self.radio_button_value.get() == "male":
            default_male = ImageTk.PhotoImage(Image.open("Characters/default_male.jpg"))
            self.hero_label.configure(image=default_male)
            self.hero_label.image = default_male
            self.hero_image_location = "Characters/default_male.jpg"
        else:
            default_female = ImageTk.PhotoImage(Image.open("Characters/default_female.jpg"))
            self.hero_label.configure(image=default_female)
            self.hero_label.image = default_female
            self.hero_image_location = "Characters/default_female.jpg"
    ...
    def add_hero(self, chair_number):
        if self.hero_name_entry.get():
            ...
            ttk.messagebox.showwarning("Oops", "Please enter the name of the Hero", parent=self)
            self.focus_set()
